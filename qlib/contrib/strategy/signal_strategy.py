...
class EvenWeightStrategy(WeightStrategyBase):
    ...
    信号语义（三值信号）：
    - score =  1：买入信号 —— 无论当前是否持有，进入目标持仓池
    - score =  0：中性信号 —— 「若当前已持有则继续持有，否则不新买」
                  即 score=0 不会触发卖出，也不会触发新买入
    - score = -1：卖出信号 —— 不进入目标持仓池，已持有则自动平仓

    持仓分配（滚动再平衡）：
    - 目标持仓池 = score=1 的标的 + 当前持有且 score=0 的标的
    - 池内所有股票等权重分配（受 max_stock_weight 约束）
    - score=-1 的标的自动清仓

    注意：score=0 对未持有标的无影响（不新买），只保护已持有的仓位。
    适配此策略的模型应在「持有区间」持续输出 score=1，
    或在「持有但不确定」时输出 score=0（保仓），在「明确退出」时输出 score=-1。
