...
def guess_plotly_rangebreaks(dt_index: pd.DatetimeIndex):
    ...
    # Convert pd.Timestamp values to ISO-format strings for JSON serialization
    # (kaleido>=1.0.0 requires JSON-serializable values in layout dicts)
    return [
        dict(values=[v.isoformat() for v in vals], dvalue=int(k.total_seconds() * 1000))
        for k, vals in gaps_to_break.items()
    ]
